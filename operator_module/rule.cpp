...
const shared_ptr<Zone> &Rule::getZone() const
{
    return zone;
}

void Rule::setZone(const shared_ptr<Zone> &newZone)
{
    zone = newZone;
}
...
const std::vector<shared_ptr<SystemReaction> > &Rule::getListSystemReaction() const
{
    return listSystemReaction;
}

void Rule::setListSystemReaction(const std::vector<shared_ptr<SystemReaction> > &newListSystemReaction)
{
...
Rule::Rule(std::string json)
{
    ...
    {
        shared_ptr<SystemReaction> reaction(SystemReaction::create(item.toString().toStdString()));
        listSystemReaction.push_back(reaction);
    }
    zone = make_shared<Zone>(obj["zone"].toString().toStdString());
}
...
std::string Rule::toJson()
{
    ...
    for (auto reaction : listSystemReaction)
